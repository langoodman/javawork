...
public class BaseController {
	...
	public User getUser(HttpServletRequest request) {
		...
		/*
		 * String token = CookieAndSessionUtil.getCookie(request,
		 * siteConfig.getCookieConfig().getName()); // ValueOperations<String, String>
		 * opsForValue = // stringRedisTemplate.opsForValue();
		 * 
		 * // User sessionUser = CookieAndSessionUtil.getSession(request, "user"); try {
		 * if (token != null) { token = Base64Util.decode(token); // String redisUser =
		 * opsForValue.get(token); String redisUser = redisService.getString(token); if
		 * (redisUser != null) { return JsonUtil.jsonToObject(redisUser, User.class); }
		 * else { return CookieAndSessionUtil.getSession(request, "user"); } } else {
		 * return CookieAndSessionUtil.getSession(request, "user"); } } catch (Exception
		 * e) { e.printStackTrace(); return null; }
		 */
	}

	public String isLogin(HttpServletRequest request, String errorPage, String suesscePage) {
		String cookie = CookieAndSessionUtil.getCookie(request, "use");
		User user = null;
		if (cookie == null) {
