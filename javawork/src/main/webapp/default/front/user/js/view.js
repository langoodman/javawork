	...
	$.ajax({
		...
		success : function(data) {
			...
			for(var i = 0;i < data.data.list.length; i++){
				...
				newDate.setTime(parseInt(data.data.list[i].createDate));
				...
				$(".itemList").append("<div class=\"panel-body paginate-bot\"\
					...
					<span style=\"font-size:16px;\"><i class=\"glyphicon glyphicon-thumbs-up\"></i> "+data.data.list[i].postGoodCount+"</span>\
					</div>\
					<div class=\"tip\">\
					<p>\
					<span class=\"dist\"><a href=\"/n/"+data.data.list[i].nodeTitle+"\" class=\"node\">"+data.data.list[i].nodeTitle+"</a></span>\
					<span>•</span>\
					<a href=\"/user/"+data.data.list[i].author+"\" class=\"author_name dist\">"+data.data.list[i].author+"</a>\
					<span class=\"hidden-sm hidden-xs dist\">•</span>\
					<span class=\"dist\">"+str+"</span>\
					</p>\
					</div>\
					</div>\
					<div class=\"media-body\">\
					<p style=\"font-size:15px;float:right;line-height:15px;\">\
					<span class=\"hidden-sm hidden-xs\" style=\"text-align:center;display:inline-block;padding:12px;\"><span style=\"font-size:29px;\">"+data.data.list[i].viewCount+"</span><small>浏览</small></span>\
					<span class=\"hidden-sm hidden-xs\"><a href=\"/topic/"+data.data.list[i].topicId+"\" style=\"text-align:center;display:inline-block;background:#337ab7;color:white;padding:12px;\"><span style=\"font-size:29px;color:white;\">"+data.data.list[i].replyCount+"</span><small>评论</small></a></span>\
					</p>\
					</div>\
					</div>\
					</div>");  
		...
		success : function(data) {
			...
			for(var i = 0;i < data.data.list.length; i++){
				...
				$(".table-striped").append("<tr>\
					<td><span style=\"width:195px;display: inline-block;\">"+str+"</span> 评论了 <a\
					href=\"/user/"+data.data.list[i].author+"\" class=\"label label-primary\" style=\"background-color:#337ab7;\"><span style=\"width:80px;display: inline-block;\">"+data.data.list[i].author+"<span></a> 创建的话题 › <a\
					href=\"/topic/"+data.data.list[i].topicId+"\" class=\"label label-primary\" style=\"background-color:#337ab7;\">"+data.data.list[i].title+"</a></td>\
					</tr>");
			}
			$(".itemList").append("<div class=\"panel-footer\" id=\"paginate\"></div>");
			paginate(data.data.totalRow,data.data.pageSize,pageNumber,"#");
					...
					success : function(data2) {
						if (data2.success != null && data2.success == true) {
							...
							$(".fans-ul").append("\
								...
								<div data-v-dc1504f6=\"\" class=\"username\" title=\""+data.data.list[i].userId+"\">"+data.data.list[i].userName+"</div>\
								<div data-v-dc1504f6=\"\" class=\"detail\">"+data.data.list[i].signature+"</div>\
								</div><button data-v-dc1504f6=\"\" class=\"follow-btn\">未关注</button>\
								</a>\
								</div>\
								</li>");
						}
