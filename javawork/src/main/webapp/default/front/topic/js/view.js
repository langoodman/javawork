...
function func1(){
	var upordown =$("#upDown").text();
	if( upordown == "up" ){
		document.getElementById("clickup").style.background = "rgba(224, 160, 158, 0.53)";
    	document.getElementById("clickdown").style.background = "white";
	}
	else if( upordown == "down" ){
		document.getElementById("clickup").style.background = "white";
    	document.getElementById("clickdown").style.background = "rgba(224, 160, 158, 0.53)";
	}
	else{
		document.getElementById("clickup").style.background = "white";
    	document.getElementById("clickdown").style.background = "white";
...
function voteTopic(tid, action) {
    $.ajax({
        ...
        success: function (data) {
        	var upDownEntity = data.data.data;
        	//console.log(upDownEntity);
        	 if (data.success != null && data.success == true) {
                 upCount();
                 downCount();
                 if( upDownEntity.upDown == true && upDownEntity.delete == false ){
                  	document.getElementById("clickup").style.background = "rgba(224, 160, 158, 0.53)";
                  	document.getElementById("clickdown").style.background = "white";
                  }
                  else if( upDownEntity.upDown == false && upDownEntity.delete == false ){
                 	document.getElementById("clickdown").style.background = "rgba(224, 160, 158, 0.53)";
                 	document.getElementById("clickup").style.background="white";
                 }
                 else{
...
$(function () {
    ...
    $("#wangEditor-btn").click(function () {
    	var contentHtml=CKEDITOR.instances.ckeditor.getData();
